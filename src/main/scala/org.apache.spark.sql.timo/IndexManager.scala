...
private[timo] class IndexManager extends Logging with Serializable {
  ...
  private[timo] def loadIndex(timoSession: TimoSession, indexName: String, fileName: String): Unit = {
    ...
    val up_bounds = sparkContext.objectFile[Long]("hdfs://master:9000/index/" + fileName + "/bounds").collect()

    val rdd = sparkContext.objectFile[IndexedPartition]("hdfs://master:9000/index/" + fileName + "/rdd").persist()
  }
}