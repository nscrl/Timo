...
private[timo] case class IndexedRelationScan(attributes: Seq[Attribute],
  ...
  private val range_search=TimoSessionState.TimoConf.getConfString("timo.aggerator.range")
  ...
  : List[Expression] = {
    ...
    condition.foreach {now =>
        now match {
          case top  @ InTime(point:Expression,timepoint,timepoint1)=>
            ans=ans:+now
          case point @ InTemporal(_,_)=>
            ans=ans:+now
          case range @ InRange(_,_,_)=>
            ans=ans:+now
          case steid @ IntervalFind(_,_,_,_)=>
            ans=ans:+now
          case max  @ InMax(_,_,_,_)=>
            ans=ans:+now
          case min  @ InMin(_,_,_,_)=>
            ans=ans:+now
          case mean @ InMean(_,_,_,_)=>
            ans=ans:+now
          case _ =>
            Nil
      }
  ...
  override protected def doExecute(): RDD[InternalRow] = {
    ...
    var accmulator_long=new AccumulatorLong
      ...
      case hash @ HashIndexRelation(_,_,_,_,_)=>
        if(predicates.nonEmpty){
          predicates.map(predicates=>{
            ...
            exps.foreach({
              case InTemporal(temporal:Expression,timepoint:Literal)=>
              ...
              case InRange(attr:Expression,st:Literal,ed:Literal)=>
              ...
              case InMax(attr:Expression,st:Literal,ed:Literal,classify:Literal)=>
                flag_agge=classify.toString().toInt
                query_time(0)=st.value.asInstanceOf[Long]
                query_time(1)=ed.value.asInstanceOf[Long]
                query_sets=get_Partition(query_time(0),query_time(1),bounds)

              case InMin(attr:Expression,st:Literal,ed:Literal,classify:Literal)=>
                flag_agge=classify.toString().toInt
                query_time(0)=st.value.asInstanceOf[Long]
                query_time(1)=ed.value.asInstanceOf[Long]
                query_sets=get_Partition(query_time(0),query_time(1),bounds)

              case InMean(attr:Expression,st:Literal,ed:Literal,classify:Literal)=>
                flag_agge=classify.toString().toInt
                query_time(0)=st.value.asInstanceOf[Long]
                query_time(1)=ed.value.asInstanceOf[Long]
                query_sets=get_Partition(query_time(0),query_time(1),bounds)
            ...
            val aggeresult: ArrayBuffer[Int] = new ArrayBuffer[Int]()
          ...
          if(predicates.nonEmpty){
            predicates.map(predicates=>{
              ...
              exps.foreach({
                ...
                case IntervalFind(attr:Expression,st:Literal,ed:Literal,_)=>
                  query_time(0)=st.value.asInstanceOf[Long]
                  query_time(1)=ed.value.asInstanceOf[Long]
                  var getBound:Boolean=false
                  query_sets=get_Partition(query_time(0),query_time(1),bounds)
                  single_attr=false
              ...
              val res=pruned.flatMap(packed => {
                ...
                else if(mid_partition_flag==true){
                  data
                }else {
