...
/**
  * Created by houkailiu on 6/15/16.
  */
object RangePartition{

  def sortBasedShuffleOn: Boolean = SparkEnv.get.shuffleManager.isInstanceOf[SortShuffleManager]

  def apply(origin: RDD[(Long,InternalRow)], num_partitions: Int)
  : (RDD[(Long,InternalRow)], Array[Long]) = {
    val rdd = if (sortBasedShuffleOn) {
      origin.mapPartitions { iter => iter.map(row => {
        (row._1, row._2.copy())
      })}
    } else {
    ...
    val shuffled=new ShuffledRDD[Long,InternalRow,InternalRow](rdd, part)
...
class RangePartitioner[K : Ordering : ClassTag, V](partitions: Int,
  ...
  private[timo] var rangeBounds: Array[K] = {
    if (partitions <= 1) {
      Array.empty
    } else {
      // This is the sample size we need to have roughly balanced output partitions, capped at 1M.
      val sampleSize = math.min(5.0 * partitions, 1e6)
...
private[timo] object RangePartitioner {
  /**
  ...
  def determineBounds[K : Ordering : ClassTag](candidates: ArrayBuffer[(K, Float)], partitions: Int): Array[K] = {
    ...
    while ((i < numCandidates) && (j < partitions - 1)) {
      ...
      cumWeight += weight
      //> -> = make the partition more balanced
      if (cumWeight >= target) {
    ...
    bounds+=ordered(ordered.length-1)._1
    bounds.toArray
  }
}