...
class Dataset[T] private[timo](@transient val TimoSession: TimoSession,
  ...
  //Search operations
  ////////////////////////////////////////
  def Range_Find(records:String,start_point:Long,end_point:Long):Dataset[Row]=withPlan {
  ...
  def Interval_Find(records:Array[String],start_point:Long,end_point:Long):Dataset[Row] =withPlan{
  ...
  def Temporal_Find(records:Array[String],timepoint:Long): Dataset[Row] =withPlan{
    val attrs=getAttributes(records)
    attrs.foreach(attr=>assert(attr!=null,"column not found"))
  ...
  ////////////////////////////////////////
  def Max(cols:String,timepoint:Long,timepoint1:Long):Dataset[Row]=withPlan{
  ...
  def Min(cols:String,timepoint:Long,timepoint1:Long):Dataset[Row]=withPlan{
    val attr=getAttributes(Array(cols))
    TimoSession.sessionState.setConf("timo.aggerator.order",attrs.indexWhere(_.name == cols).toString)
    attr.foreach(attr=>assert(attr!=null,"Column not found"))
  ...
  def Mean(cols:String,timepoint:Long,timepoint1:Long):Dataset[Row]=withPlan{
    val attr=getAttributes(Array(cols))
    TimoSession.sessionState.setConf("timo.aggerator.order",attrs.indexWhere(_.name == cols).toString)
    attr.foreach(attr=>assert(attr!=null,"Column not found"))
  ...
  def slotTopk(key:String,startpoint:Long,endpoint:Long,k:Int) : Dataset[Row] = withPlan {
  ...
  def TopK(records:Array[String],timepoint:Long,timepoint1:Long,k:Int): Dataset[Row] =withPlan{
    val attrs = getAttributes(records)
    attrs.foreach(attr => assert(attr != null, "column not found"))
  ...
  @inline private def withPlan(logicalPlan: => LogicalPlan): Dataset[Row] = {
