...
private[timo] case class IntervalTreeIndexRelation(
  ...
  override def newInstance(): IntervalTreeIndexRelation.this.type = {
    IntervalTreeIndexRelation(output.map(_.newInstance()), child, tableName, columnKeys, indexName)(_indexedRDD).asInstanceOf[this.type]
  }
  ...
  override var temporalRDD: TemporalRDD[Long] = _
}
