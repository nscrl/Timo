...
class STEIDIndex[T] extends Index with Serializable{
  ...
  def get_ID_Result(start:Long,end:Long): ArrayBuffer[Int] ={
    ...
    val bound = bounds.indexWhere(iter => iter > start.toInt) - 1
    val ed = get_ED_Result((bound * num).toInt, start.toInt)
...
object STEIDIndex{
  ...
  def apply[T](data:Array[InternalRow]):STEIDIndex[T] = {
    ...
    index.mintime=min
    index.maxtime=max
    if(TimoConf.INDEX_ATTR_NUM.toString.toInt==1) {
    ...
    val num = (calcu_internal_time(data) * 1).toLong
        ...
        if (!index.single_double) {
          ...
          if (index.ED.containsKey(end.toInt)) {
            try {
              ...
              case _:NullPointerException => println("debug")
            }
          } else {
