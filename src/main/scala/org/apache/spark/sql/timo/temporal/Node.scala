...
class Node[T: ClassTag](value:Array[Long], time:Array[Long]) extends Serializable {
  ...
  val minValue: Long =value(0)
  val maxValue: Long =value(1)

  val minTime:Long=time(0)
  val maxTime:Long=time(1)

  var NodeMax: Long =time(1)
  var NodeMin: Long =time(0)
  ...
  def multiput(rs: Iterator[T]): ListBuffer[T] = {
    val newData = rs.toList
    data ++= newData
  }

  def multiput(rs: List[T]): ListBuffer[T] = {
    data ++= rs
  }

  def put(newData: T): ListBuffer[T] = {
    data += newData
  }
